 ...
 */
export function scanPackageDts(packageName, basePath, visited) {
    ...
    if (visited) {
        for (const p of referencedPackages) {
            if (p === packageName || visited.has(p))
                continue;
            visited.add(p);
            try {
 ...
 * skipped.
 */
export function collectImportedPackages(code, fileName, program) {
 ...
 */
export function collectRelativeReExports(code, fileName, program) {
    if (!program) {
        try {
            program = parseSync(fileName, code).program;
        }
 ...
 */
export function collectAllImports(code, fileName, program) {
    if (!program) {
        try {
            program = parseSync(fileName, code).program;
        }
