...
export function collectDeferBlocks(nodes) {
    const result = [];
    function walk(node) {
        if (!node)
            return;
        ...
        if (node.empty?.children)
            node.empty.children.forEach(walk);
        if (node.placeholder?.children)
            node.placeholder.children.forEach(walk);
        if (node.loading?.children)
            node.loading.children.forEach(walk);
        if (node.error?.children)
            node.error.children.forEach(walk);
    }
    nodes.forEach(walk);
    return result;
}
 ...
 */
export function collectElementNames(nodes, excludeDeferBlocks = false) {
 ...
 * imports.
 */
export function buildImportMap(sf) {
    ...
    for (const stmt of sf.statements) {
        ...
        if (clause.namedBindings && ts.isNamedImports(clause.namedBindings)) {
            for (const el of clause.namedBindings.elements) {
                ...
                const info = { path, isDefault: false, exportName };
 ...
 */
export function buildDeferDependencyMap(parsedTemplate, sourceFile, registry, localSelectors) {
    ...
    if (registry) {
        for (const [className, entry] of registry) {
            if (entry.selector)
                selectorToClass.set(entry.selector, className);
        }
    }
    ...
    const deferOnlyElements = new Set();
    for (const el of deferElements) {
        if (!eagerElements.has(el))
            deferOnlyElements.add(el);
    }
