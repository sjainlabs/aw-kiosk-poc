...
function extractAlias(optArg) {
    if (!optArg || optArg.type !== 'ObjectExpression')
        return null;
...
export function buildPropDecorators(classNode, sourceCode) {
    ...
    for (const member of members) {
        ...
        const explicitDecoratorNames = new Set();
        ...
        if (member.type === 'PropertyDefinition' &&
            ...
            if (api === 'input') {
                ...
                const alias = extractAlias(optArg);
                if (alias !== null) {
                    props[memberName].push(`{type: Output, args: ['${escapeStringLiteral(alias)}']}`);
                }
                else {
                ...
                if (!props[memberName])
                    props[memberName] = [];
                const queryType = api === 'viewChildren'
                    ? 'ViewChildren'
                    : api === 'viewChild'
                        ? 'ViewChild'
                        : api === 'contentChildren'
                ...
                if (args.length > 0) {
                    ...
                    props[memberName].push(`{type: ${queryType}, args: [undefined, {isSignal: true}]}`);
                }
            }
        }
    }
