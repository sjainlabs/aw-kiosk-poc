...
function countInlineStyleLiterals(arrayExpr) {
    ...
    for (const el of arrayExpr.elements ?? []) {
        if (el?.type === 'Literal' && typeof el.value === 'string')
            count++;
        else if (el?.type === 'TemplateLiteral' && el.quasis?.length === 1)
            count++;
    }
    return count;
}
 ...
 */
export async function inlineResourceUrls(code, fileName) {
    ...
    await Promise.all([...pathsToRead].map(async (p) => {
        fileContents.set(p, await fs.promises.readFile(p, 'utf-8').catch(() => undefined));
    }));
    ...
    for (const plan of plans) {
        ...
        if (cssProps.length > 0) {
            ...
            allExtensions.forEach((ext, k) => {
                if (ext)
                    styleExtensions.set(externalBase + k, ext);
            });
    ...
    return {
        ...
        styleExtensions,
        resourceDependencies: [...pathsToRead].filter((p) => fileContents.get(p) !== undefined),
    };
}
 ...
 */
export function extractInlineStyles(code, fileName, program) {
