...
import { createPersistentTransformCache, resolveTransformCacheDir, } from './transform-cache.js';
/**
 ...
 */
export function isProdMode(mode) {
    if (mode === 'development') {
        return false;
    }
    return mode === 'production' || process.env['NODE_ENV'] === 'production';
}
export function getTsConfigPath(root, tsconfig, isProd, isTest, isLib, workspaceRoot) {
    ...
    const attemptedPaths = workspacePath && workspacePath !== resolvedPath
...
export function createDepOptimizerConfig(opts) {
    ...
    const rolldownOptions = {
        plugins: [
            ...
            }, opts.isTest, !opts.isAstroIntegration, transformCache),
        ],
    };
