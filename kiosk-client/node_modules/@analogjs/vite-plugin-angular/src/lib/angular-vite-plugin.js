...
import { applyLiveReloadCompilerOptions, shouldDiscardIncrementalProgram, shouldEnableExternalRuntimeStyles, } from './live-reload-compiler-options.js';
import { encapsulateComponentStylesPlugin, getComponentStyleSheetMeta, getFilenameFromPath, isComponentStyleSheet, } from './encapsulate-component-styles-plugin.js';
...
export function angular(options) {
    ...
    function angularPlugin() {
        ...
        return {
            ...
            async buildStart() {
                if (!jit) {
                    styleTransform = (code, filename) => preprocessCSS(code, filename, resolvedConfig);
                }
                // Defer the first compilation in test mode
                if (!isVitestVscode) {
                    pendingCompilation = performCompilation(this.environment?.config ?? resolvedConfig);
                    ...
                    if (isTest) {
                        ...
                        if (tsMod) {
                            ...
                            if (testWatchMode && invalidated) {
                                pendingCompilation = performCompilation(this.environment?.config ?? resolvedConfig, [id]);
                            }
                        }
                    }
    ...
    function normalizeSourceMapSources(map, id) {
    ...
    async function performAngularCompilation(config, ids) {
        ...
        const compilationResult = await angularCompilation.initialize(resolvedTsConfigPath, {
            ...
            async transformStylesheet(data, containingFile, resourceFile, order, className) {
                ...
                try {
                    stylesheetResult = await getStyleTransform()(data, `${filename}?direct`);
                }
                catch (e) {
                    console.error(`${e}`);
                }
    ...
    async function _doPerformCompilation(config, ids) {
        ...
        const discardIncrementalProgram = shouldDiscardIncrementalProgram({
            ...
            incrementalProgramUsesExternalRuntimeStyles: liveReloadProgramHasExternalStyles,
        });
        ...
        const beforeTransformers = jit
            ? [
                compilerCli.constructorParametersDownlevelTransform(emitBuilder.getProgram()),
                createJitResourceTransformer(() => emitBuilder.getProgram().getTypeChecker()),
            ]
            : [];
        const transformers = mergeTransformers({ before: beforeTransformers }, jit ? {} : angularCompiler.prepareEmit().transformers);
        const fileMetadata = getFileMetadata(emitBuilder, angularCompiler, pluginOptions.liveReload, pluginOptions.disableTypeChecking);
        ...
        const writeOutputFile = (id) => {
            ...
            let map;
            let mapFilename;
            emitBuilder.emit(sourceFile, (filename, data) => {
        ...
        if (watchMode) {
            if (ids && ids.length > 0) {
                ...
                if (isTest) {
                    ...
                    while (emitBuilder.emitNextAffectedFile(writeFileCallback, undefined, undefined, transformers)) {
                        /* empty */
                    }
                }
            }
